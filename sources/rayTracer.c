...
/*   Updated: 2022/10/05 19:19:49by cbridget         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	traceRay(t_minirt *data, t_coordinates *ray)
{
	...
	asw = closestIntersection(data, &data->camera.crdn, ray, 1.0f);
	if (asw.closest_sphere == NULL)
		return (create_trgb(0, 0, 0));
	pl = multiplicationScalar(ray, asw.closest_t);
...
t_answer	closestIntersection(t_minirt *data, t_coordinates *v1, t_coordinates *v2, float t_min)
{
	...
	while (sphere_list)
	{
		...
		if (asw.t1 >= t_min && asw.t1 < asw.closest_t)
		{
			asw.closest_t = asw.t1;
			asw.closest_sphere = sphere_list->content;
		}
		if (asw.t2 >= t_min && asw.t2 < asw.closest_t)
		{
